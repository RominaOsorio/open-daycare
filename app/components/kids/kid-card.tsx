...
export function KidCard({
  ...
  const cardClass =
    "flex items-center gap-3.5 rounded-[18px] border border-borde bg-tarjeta p-4 shadow-[0_4px_14px_-12px_rgba(120,90,60,.5)]";
  ...
  return (
    <Link
      href={`/kids/${kid.slug}`}
      className={`${cardClass} transition hover:-translate-y-0.5 hover:border-[#f2a78e]`}
    >
      {content}
    </Link>
  );
